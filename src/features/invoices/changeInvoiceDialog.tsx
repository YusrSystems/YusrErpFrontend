...
export default function ChangeInvoiceDialog({
  ...
  const { formData, handleChange, getError, isInvalid, validate } =
    useEntityForm<Invoice>(initialValues, validationRules);
      ...
      <div className="flex flex-col gap-6">
        ...
        <FieldsSection title="البيانات الأساسية" columns={2}>
          <SelectField
            ...
            onValueChange={(val) =>
              handleChange({ type: Number(val) as InvoiceType })
            }
        ...
        <FieldsSection title="معلومات إضافية" columns={2}>
          ...
          <SelectField
            ...
            options={[
              { label: "محلي", value: ImportExportType.Local.toString() },
              { label: "تصدير", value: ImportExportType.Export.toString() },
              {
                label: "استيراد (آلية الاحتساب العكسي)",
                value:
                  ImportExportType.ImportAccordingToTheReverseChargeMechanism.toString(),
              },
              {
