...
import { Button, ChangeDialog, FieldGroup, FieldsSection, Input, NumberField, SearchableSelect, SelectField, TextAreaField, TextField, useEntityForm } from "@yusr_systems/ui";
...
export default function ChangeAccountDialog({ entity, mode, service, onSuccess }: CommonChangeDialogProps<Account>)
{
  ...
  const initialValues = useMemo(() => ({
    type: entity?.type || AccountType.Client,
    ...entity,
    name: entity?.name || "",
  ...
  const requiresTaxInfo = formData?.type === AccountType.Client || formData?.type === AccountType.Supplier
      ...
      <div className="max-h-[75vh] overflow-y-auto px-2 pb-2">
        <FieldGroup>
          ...
          { (requiresTaxInfo || isBank) && (
            <FieldsSection title={ isBank ? "المعلومات البنكية" : "المعلومات الضريبية والتجارية" } columns={ 2 }>
              { requiresTaxInfo && (
                <>
                  <TextField
                  ...
                  <TextField
                    ...
                    value={ formData.crn || "" }
                    onChange={ (e) =>
                      handleChange({ crn: e.target.value }) }
                    dir="ltr"
                  />
                </>
              ) }
          ...
          { (requiresAddress || requiresContacts) && (
            <div
              className={ `grid gap-6 ${
                requiresAddress && requiresContacts ? "grid-cols-1 md:grid-cols-2" : "grid-cols-1"
              }` }
            >
              { requiresAddress && (
